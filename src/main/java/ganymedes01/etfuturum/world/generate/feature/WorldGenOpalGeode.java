...
public class WorldGenOpalGeode extends WorldGenerator {
	...
	private final double buddingOpalChance;//Formerly known as geodeFeatureConfig.useAlternateLayer0Chance
	private final double usePotentialPlacementsChance;//geodeFeatureConfig.usePotentialPlacementsChance
	
	public WorldGenOpalGeode() {
		this(-16, 16, 1, new int[] {3, 4}, new int[] {4, 5, 6}, 1.7D, 2.2D, 3.2D, 4.2D, new int[] {1, 2}, 0.95D, 2.0D, 2, 0.05D, 0.083D, 0.35D);
	}
	
	public WorldGenOpalGeode(int minOffset, int maxOffset, int invalidMax, int[] distPoints, int[] outerWallDist, double fill, double inner, double middle, double outer, int[] pointOff, double crackChance, double baseCrack, int crackPointOff, double noiseAmp, double budChance, double potentialPlaceChance) {
		outerMeta = ConfigWorld.gemstoneOuterBlock == ExternalContent.netherlicious_basalt_bricks ? 6 : 0;
		budBlocks = ImmutableList.of(ModBlocks.opal_cluster_1, ModBlocks.opal_cluster_2);
		...
		buddingOpalChance = budChance;
		usePotentialPlacementsChance = potentialPlaceChance;
	}
	
	/**
	 * This is used when generating opal so it doesn't generate in the middle of the air, ocean, hanging in trees, etc.
	 */
			 ...
			 while(true) {
				...
				if(world.getBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ()).getBlockHardness(world, blockPos3.getX(), blockPos3.getY(), blockPos3.getZ()) != -1) {
					if (bl && v >= l && u < fillingSqrt) {
					   ...
					   boolean bl2 = (double)random.nextFloat() < this.buddingOpalChance;
					   if (bl2) {
						   world.setBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ(), ModBlocks.budding_opal);//AlternateInnerLayerProvider
					   } else {
						   world.setBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ(), ModBlocks.opal_block);//InnerLayerProvider
					   }
