...
public class WorldGenSapphireGeode extends WorldGenerator {
	...
	private final double buddingSapphireChance;//Formerly known as geodeFeatureConfig.useAlternateLayer0Chance
	private final double usePotentialPlacementsChance;//geodeFeatureConfig.usePotentialPlacementsChance
	
	public WorldGenSapphireGeode() {
		this(-16, 16, 1, new int[] {3, 4}, new int[] {4, 5, 6}, 1.7D, 2.2D, 3.2D, 4.2D, new int[] {1, 2}, 0.95D, 2.0D, 2, 0.05D, 0.083D, 0.35D);
	}
	
	public WorldGenSapphireGeode(int minOffset, int maxOffset, int invalidMax, int[] distPoints, int[] outerWallDist, double fill, double inner, double middle, double outer, int[] pointOff, double crackChance, double baseCrack, int crackPointOff, double noiseAmp, double budChance, double potentialPlaceChance) {
		outerMeta = ConfigWorld.gemstoneOuterBlock == ExternalContent.netherlicious_basalt_bricks ? 6 : 0;
		budBlocks = ImmutableList.of(ModBlocks.sapphire_cluster_1, ModBlocks.sapphire_cluster_2);
		...
		buddingSapphireChance = budChance;
		usePotentialPlacementsChance = potentialPlaceChance;
	}
	
	/**
	 * This is used when generating sapphire so it doesn't generate in the middle of the air, ocean, hanging in trees, etc.
	 */
			 ...
			 while(true) {
				...
				if(world.getBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ()).getBlockHardness(world, blockPos3.getX(), blockPos3.getY(), blockPos3.getZ()) != -1) {
					if (bl && v >= l && u < fillingSqrt) {
					   ...
					   boolean bl2 = (double)random.nextFloat() < this.buddingSapphireChance;
					   if (bl2) {
						   world.setBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ(), ModBlocks.budding_sapphire);//AlternateInnerLayerProvider
					   } else {
						   world.setBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ(), ModBlocks.sapphire_block);//InnerLayerProvider
					   }
