...
import ganymedes01.etfuturum.items.block.ItemTopazCluster;
...
public class BlockTopazCluster extends BlockTopazBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockTopazCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("topaz_cluster");
		setBlockName(Utils.getUnlocalisedName("topaz_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.topaz_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_topaz_bud");
			icons[1] = p_149651_1_.registerIcon("medium_topaz_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_topaz_bud");
	...
	public int getRenderType() {
		return RenderIDs.TOPAZ_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemTopazCluster.class;
	}
