...
import ganymedes01.etfuturum.items.block.ItemRubyCluster;
...
public class BlockRubyCluster extends BlockRubyBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockRubyCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("ruby_cluster");
		setBlockName(Utils.getUnlocalisedName("ruby_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.ruby_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_ruby_bud");
			icons[1] = p_149651_1_.registerIcon("medium_ruby_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_ruby_bud");
	...
	public int getRenderType() {
		return RenderIDs.RUBY_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemRubyCluster.class;
	}
