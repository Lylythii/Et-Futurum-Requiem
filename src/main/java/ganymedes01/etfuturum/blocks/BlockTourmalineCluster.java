...
import ganymedes01.etfuturum.items.block.ItemTourmalineCluster;
...
public class BlockTourmalineCluster extends BlockTourmalineBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockTourmalineCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("tourmaline_cluster");
		setBlockName(Utils.getUnlocalisedName("tourmaline_cluster_" + (type + 1)));
	...
	{
		return ModItems.tourmaline_shard;
	}
	...
	{
		if(this == ModBlocks.tourmaline_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_tourmaline_bud");
			icons[1] = p_149651_1_.registerIcon("medium_tourmaline_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_tourmaline_bud");
	...
	public int getRenderType() {
		return RenderIDs.TOURMALINE_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemTourmalineCluster.class;
	}
