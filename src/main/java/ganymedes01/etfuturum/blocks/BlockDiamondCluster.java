...
import ganymedes01.etfuturum.items.block.ItemDiamondCluster;
...
public class BlockDiamondCluster extends BlockDiamondBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockDiamondCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("diamond_cluster");
		setBlockName(Utils.getUnlocalisedName("diamond_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.diamond_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_diamond_bud");
			icons[1] = p_149651_1_.registerIcon("medium_diamond_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_diamond_bud");
	...
	public int getRenderType() {
		return RenderIDs.DIAMOND_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemDiamondCluster.class;
	}
