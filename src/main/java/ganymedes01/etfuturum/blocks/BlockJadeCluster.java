...
import ganymedes01.etfuturum.items.block.ItemJadeCluster;
...
public class BlockJadeCluster extends BlockJadeBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockJadeCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("jade_cluster");
		setBlockName(Utils.getUnlocalisedName("jade_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.jade_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_jade_bud");
			icons[1] = p_149651_1_.registerIcon("medium_jade_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_jade_bud");
	...
	public int getRenderType() {
		return RenderIDs.JADE_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemJadeCluster.class;
	}
