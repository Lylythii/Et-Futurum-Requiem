...
public class BlockBuddingGarnet extends BlockGarnetBlock {
	
	public BlockBuddingGarnet() {
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneBlock : soundTypeGlass);
		setBlockTextureName("budding_garnet");
		setBlockName(Utils.getUnlocalisedName("budding_garnet"));
	...
	public void updateTick(World world, int x, int y, int z, Random rand) {
		if (rand.nextInt(5) == 0) {
			...
			if(block instanceof BlockGarnetCluster && meta % 6 == facing.ordinal()) {
				if(meta < 6) {
					world.setBlockMetadataWithNotify(x + facing.getFrontOffsetX(), y + facing.getFrontOffsetY(), z + facing.getFrontOffsetZ(), meta + 6, 3);
				} else if(block == ModBlocks.garnet_cluster_1) {
					world.setBlock(x + facing.getFrontOffsetX(), y + facing.getFrontOffsetY(), z + facing.getFrontOffsetZ(), ModBlocks.garnet_cluster_2, meta - 6, 3);
				}
			} else if(canGrowIn(block)) {
				world.setBlock(x + facing.getFrontOffsetX(), y + facing.getFrontOffsetY(), z + facing.getFrontOffsetZ(), ModBlocks.garnet_cluster_1, facing.ordinal(), 3);
			}
		}
	}
