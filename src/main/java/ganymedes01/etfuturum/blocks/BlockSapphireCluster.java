...
import ganymedes01.etfuturum.items.block.ItemSapphireCluster;
...
public class BlockSapphireCluster extends BlockSapphireBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockSapphireCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("sapphire_cluster");
		setBlockName(Utils.getUnlocalisedName("sapphire_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.sapphire_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_sapphire_bud");
			icons[1] = p_149651_1_.registerIcon("medium_sapphire_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_sapphire_bud");
	...
	public int getRenderType() {
		return RenderIDs.SAPPHIRE_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemSapphireCluster.class;
	}
