...
import ganymedes01.etfuturum.items.block.ItemGarnetCluster;
...
public class BlockGarnetCluster extends BlockGarnetBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockGarnetCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("garnet_cluster");
		setBlockName(Utils.getUnlocalisedName("garnet_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.garnet_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_garnet_bud");
			icons[1] = p_149651_1_.registerIcon("medium_garnet_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_garnet_bud");
	...
	public int getRenderType() {
		return RenderIDs.GARNET_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemGarnetCluster.class;
	}
