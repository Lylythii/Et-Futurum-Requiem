...
import ganymedes01.etfuturum.items.block.ItemEmeraldCluster;
...
public class BlockEmeraldCluster extends BlockEmeraldBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockEmeraldCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("emerald_cluster");
		setBlockName(Utils.getUnlocalisedName("emerald_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.emerald_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_emerald_bud");
			icons[1] = p_149651_1_.registerIcon("medium_emerald_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_emerald_bud");
	...
	public int getRenderType() {
		return RenderIDs.EMERALD_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemEmeraldCluster.class;
	}
