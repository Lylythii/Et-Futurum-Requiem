...
import ganymedes01.etfuturum.items.block.ItemBerylCluster;
...
public class BlockBerylCluster extends BlockBerylBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockBerylCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("beryl_cluster");
		setBlockName(Utils.getUnlocalisedName("beryl_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.beryl_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_beryl_bud");
			icons[1] = p_149651_1_.registerIcon("medium_beryl_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_beryl_bud");
	...
	public int getRenderType() {
		return RenderIDs.BERYL_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemBerylCluster.class;
	}
