...
import ganymedes01.etfuturum.items.block.ItemOpalCluster;
...
public class BlockOpalCluster extends BlockOpalBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockOpalCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("opal_cluster");
		setBlockName(Utils.getUnlocalisedName("opal_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.opal_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_opal_bud");
			icons[1] = p_149651_1_.registerIcon("medium_opal_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_opal_bud");
	...
	public int getRenderType() {
		return RenderIDs.OPAL_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemOpalCluster.class;
	}
