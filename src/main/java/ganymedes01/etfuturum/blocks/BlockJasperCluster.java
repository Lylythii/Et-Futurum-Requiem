...
import ganymedes01.etfuturum.items.block.ItemJasperCluster;
...
public class BlockJasperCluster extends BlockJasperBlock implements ISubBlocksBlock, IMultiStepSound {
	
	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockJasperCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		setStepSound(ConfigSounds.newBlockSounds ? ModSounds.soundGemstoneCluster : soundTypeGlass);
		setBlockTextureName("jasper_cluster");
		setBlockName(Utils.getUnlocalisedName("jasper_cluster_" + (type + 1)));
	...
	{
		if(this == ModBlocks.jasper_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("small_jasper_bud");
			icons[1] = p_149651_1_.registerIcon("medium_jasper_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("large_jasper_bud");
	...
	public int getRenderType() {
		return RenderIDs.JASPER_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemJasperCluster.class;
	}
